...
import MoagentbaseRetrievalModal from './moagentbase-retrieval-modal'
...
const HitTesting: FC<Props> = ({ datasetId }: Props) => {
  ...
  const [isShowMoagentbaseRetrievalModal, setIsShowMoagentbaseRetrievalModal] = useState(false)
    ...
    <div className={s.container}>
      <div className={s.leftDiv}>
        ...
        <Textarea
          ...
          setText={setText}
          text={text}
          onClickRetrievalMethod={() => setIsShowMoagentbaseRetrievalModal(true)}
      ...
      <Drawer isOpen={isShowMoagentbaseRetrievalModal} onClose={() => setIsShowMoagentbaseRetrievalModal(false)} footer={null} mask={isMobile} panelClassname='mt-16 mx-2 sm:mr-2 mb-3 !p-0 !max-w-[640px] rounded-xl'>
        <MoagentbaseRetrievalModal
          indexMethod={currentDataset?.indexing_technique || ''}
          value={retrievalConfig}
          isShow={isShowMoagentbaseRetrievalModal}
          onHide={() => setIsShowMoagentbaseRetrievalModal(false)}
          onSave={(value) => {
            setRetrievalConfig(value)
            setIsShowMoagentbaseRetrievalModal(false)
          }}
        />
      </Drawer>
    </div>
