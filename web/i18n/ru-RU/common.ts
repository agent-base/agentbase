const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Учетная запись AgentBase',
    langGeniusAccountTip: 'Ваша учетная запись AgentBase и связанные с ней пользовательские данные.',
  ...
  members: {
    ...
    invitationSentTip: 'Приглашение отправлено, и они могут войти в AgentBase, чтобы получить доступ к данным вашей команды.',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'Хостинговая служба OpenAI, предоставляемая AgentBase, позволяет вам использовать такие модели, как GPT-3.5. Прежде чем ваша пробная квота будет исчерпана, вам необходимо настроить других поставщиков моделей.',
  ...
  apiBasedExtension: {
    title: 'API-расширения обеспечивают централизованное управление API, упрощая настройку для удобного использования в приложениях AgentBase.',
    link: 'Узнайте, как разработать собственное API-расширение.',
    linkUrl: 'https://docs.agentbase.ai/features/extension/api_based_extension',
  ...
  about: {
    ...
    nowAvailable: 'AgentBase {{version}} теперь доступен.',
    latestAvailable: 'AgentBase {{version}} - последняя доступная версия.',
  },
  appMenus: {
    overview: 'Мониторинг',
