...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        :return:
        """
        url = URL(str(agentbase_config.CODE_EXECUTION_ENDPOINT)) / "v1" / "sandbox" / "run"

        headers = {"X-Api-Key": agentbase_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=agentbase_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=agentbase_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=agentbase_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
