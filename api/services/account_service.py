...
from models.model import AgentBaseSetup
...
class RegisterService:
    ...
    def setup(cls, email: str, name: str, password: str, ip_address: str) -> None:
        ...
        try:
            ...
            agentbase_setup = AgentBaseSetup(version=agentbase_config.CURRENT_VERSION)
            db.session.add(agentbase_setup)
            db.session.commit()
        except Exception as e:
            db.session.query(AgentBaseSetup).delete()
    ...
    def register(
        ...
        try:
            ...
            if agentbase_config.EDITION != "SELF_HOSTED":
                tenant = TenantService.create_tenant(f"{account.name}'s Workspace")
    ...
    def invite_new_member(
        ...
        send_invite_member_mail_task.delay(
            ...
            to=email,
            token=token,
            inviter_name=inviter.name if inviter else "AgentBase",
    ...
    def generate_invite_token(cls, tenant: Tenant, account: Account) -> str:
        ...
        expiry_hours = agentbase_config.INVITE_EXPIRY_HOURS
