...
A AgentBase App Service-API Client, using for build a webapp by request Service-API

## Usage

First, install `agentbase-client` python sdk package:

```
pip install agentbase-client
```
...
```python
from agentbase_client import CompletionClient
...
```python
import json
from agentbase_client import ChatClient
...
```python
from agentbase_client import AgentBaseClient

api_key = "your_api_key"

# Initialize Client
agentbase_client = AgentBaseClient(api_key)
...
with open(file_path, "rb") as file:
    ...
    response = agentbase_client.file_upload("user_id", files)
