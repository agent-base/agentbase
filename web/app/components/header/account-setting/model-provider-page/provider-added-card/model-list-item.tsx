...
export type ModelListItemProps = {
  ...
  onMoagentbaseLoadBalancing?: (model: ModelItem) => void
}

const ModelListItem = ({ model, provider, isConfigurable, onConfig, onMoagentbaseLoadBalancing }: ModelListItemProps) => {
            ...
            : (isCurrentWorkspaceManager && (modelLoadBalancingEnabled || plan.type === Plan.sandbox) && !model.deprecated && [ModelStatusEnum.active, ModelStatusEnum.disabled].includes(model.status))
              ? (
                <Button
                  ...
                  onClick={() => onMoagentbaseLoadBalancing?.(model)}
                >
