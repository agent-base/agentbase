...
from configs.app_config import AgentBaseConfig
...
def test_agentbase_config(example_env_file):
    # load dotenv file with pydantic-settings
    config = AgentBaseConfig(_env_file=example_env_file)
...
def test_flask_configs(example_env_file):
    ...
    os.environ.clear()
    flask_app.config.from_mapping(AgentBaseConfig(_env_file=example_env_file).model_dump())  # pyright: ignore
    ...
    assert config["SQLALCHEMY_DATABASE_URI"] == "postgresql://postgres:@localhost:5432/agentbase"
