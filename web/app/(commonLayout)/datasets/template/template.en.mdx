...
<div>
  ...
  Service API of AgentBase authenticates using an `API-Key`.
    ...
    <Properties>
      <Property name='data' type='multipart/form-data json string' key='data'>
        - original_document_id Source document ID (optional)
          - Used to re-upload the document or moagentbase the document cleaning and segmentation configuration. The missing information is copied from the source document
...
<table className="max-w-auto border-collapse border border-slate-400" style={{ maxWidth: 'none', width: 'auto' }}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td className="p-2 border border-slate-300">The dataset name already exists. Please moagentbase your dataset name.</td>
    </tr>
    <tr>
