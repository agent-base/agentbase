...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=agentbase_config.RELYT_HOST,
                port=agentbase_config.RELYT_PORT,
                user=agentbase_config.RELYT_USER,
                password=agentbase_config.RELYT_PASSWORD,
                database=agentbase_config.RELYT_DATABASE,
            ),
            group_id=dataset.id,
        )
