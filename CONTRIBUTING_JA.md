AgentBase にコントリビュートしたいとお考えなのですね。それは素晴らしいことです。
...
このガイドは AgentBase そのものと同様に、継続的に改善されています。実際のプロジェクトに遅れをとることがあるかもしれませんが、ご理解のほどよろしくお願いいたします。

ライセンスに関しては、私たちの短い[ライセンスおよびコントリビューター規約](./LICENSE)をお読みください。また、コミュニティは[行動規範](https://github.com/agent-base/.github/blob/main/CODE_OF_CONDUCT.md)を遵守しています。

## 飛び込む前に

[既存の Issue](https://github.com/agent-base/agentbase/issues?q=is:issue+is:open) を探すか、[新しい Issue](https://github.com/agent-base/agentbase/issues/new/choose) を作成してください。私たちは Issue を 2 つのタイプに分類しています。

### 機能リクエスト

* 新しい機能要望を出す場合は、提案する機能が何を実現するものなのかを説明し、可能な限り多くのコンテキストを含めてください。[@perzeusss](https://github.com/perzeuss)は、あなた様の要望を書き出すのに役立つ [Feature Request Copilot](https://uagentbase.app/chat/MK2kVSnw1gakVwMX) を作ってくれました。気軽に試してみてください。
...
| [community feedback board](https://github.com/agent-base/agentbase/discussions/categories/feedbacks)の人気の機能リクエスト | Medium Priority |
...
以下の手順で 、AgentBaseのセットアップをしてください。
...
AgentBase を構築するには次の依存関係が必要です。それらがシステムにインストールされていることを確認してください。
...
AgentBase はバックエンドとフロントエンドから構成されています。
...
よくある問題とトラブルシューティングの手順については、[installation FAQ](https://docs.agentbase.ai/v/japanese/learn-more/faq/install-faq) を確認してください。

### 5. ブラウザで agentbase にアクセスする

設定を確認するために、ブラウザで[http://localhost:3000](http://localhost:3000)(デフォルト、または自分で設定した URL とポート)にアクセスしてください。AgentBase が起動して実行中であることが確認できるはずです。

## 開発中

モデルプロバイダーを追加する場合は、[このガイド](https://github.com/agent-base/agentbase/blob/main/api/core/model_runtime/README.md)が役立ちます。

Agent や Workflow にツールプロバイダーを追加する場合は、[このガイド](./api/core/tools/README.md)が役立ちます。

AgentBase のバックエンドとフロントエンドの概要を簡単に説明します。

### バックエンド

AgentBase のバックエンドは[Flask](https://flask.palletsprojects.com/en/3.0.x/)を使って Python で書かれています。ORM には[SQLAlchemy](https://www.sqlalchemy.org/)を、タスクキューには[Celery](https://docs.celeryq.dev/en/stable/getting-started/introduction.html)を使っています。認証ロジックは Flask-login 経由で行われます。
...
これで完了です！あなた様の PR がマージされると、[README](https://github.com/agent-base/agentbase/blob/main/README.md) にコントリビューターとして紹介されます。
