...
def migrate_knowledge_vector_database():
    ...
    skipped_count = 0
    total_count = 0
    vector_type = agentbase_config.VECTOR_STORE
    page = 1
    while True:
        try:
            datasets = (
...
def add_qdrant_doc_id_index(field: str):
    ...
    try:
        ...
        for binding in bindings:
            if agentbase_config.QDRANT_URL is None:
                raise ValueError("Qdrant URL is required.")
            qdrant_config = QdrantConfig(
                endpoint=agentbase_config.QDRANT_URL,
                api_key=agentbase_config.QDRANT_API_KEY,
                root_path=current_app.root_path,
                timeout=agentbase_config.QDRANT_CLIENT_TIMEOUT,
                grpc_port=agentbase_config.QDRANT_GRPC_PORT,
                prefer_grpc=agentbase_config.QDRANT_GRPC_ENABLED,
            )
            try:
